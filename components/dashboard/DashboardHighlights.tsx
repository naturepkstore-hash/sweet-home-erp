...
export function DashboardHighlights({ birthdays, achievements, presentChildren, totalChildren, presentStaff, totalStaff }: { birthdays: Birthday[]; achievements: Achievement[]; presentChildren: number; totalChildren: number; presentStaff: number; totalStaff: number }) {
  ...
  return <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
    ...
    <div className="rounded-xl border border-pink-200 bg-pink-50/50 p-5 shadow-xs"><div className="flex items-center gap-2"><Cake className="h-4 w-4 text-pink-600" /><h2 className="text-sm font-bold text-slate-800">Upcoming Birthdays</h2></div><div className="mt-4 space-y-3">{birthdays.length ? birthdays.map((child) => <div key={child.id} className="flex items-center justify-between text-xs"><span className="font-semibold text-slate-700">{child.fullName}</span><span className="text-[10px] text-pink-700">{formatDate(child.dateOfBirth)}</span></div>) : <p className="text-xs text-slate-500">No active child records.</p>}</div></div>
